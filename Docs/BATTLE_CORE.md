...
| **[推导]** | 由既有规则计算得出的参考数据 |
| ⬜ | **依赖未决策项**，当前为推荐口径，见 [DESIGN_DECISIONS.md](DESIGN_DECISIONS.md) |
...
> ⚠ 三人排只有唯一合法解，该排没有任何布阵决策空间。约束成因是**渲染遮挡**（卡牌宽 266px > 列距 160px），非玩法设计。
>
> D-08 后站位不再由职业隐含决定，**布阵是玩家仅存的战场表达手段之一**，此约束的代价显著上升。⬜ [D-07](DESIGN_DECISIONS.md) 推荐解除。
...
**推荐 `90s`**（v1.0 为 `180s`）。零输入观战对时长极度敏感，目标单场时长：
...
> ⚠ **风险上升**：⬜ [D-04c](DESIGN_DECISIONS.md) 将战斗压到 20~40 秒后，攒满法力的窗口更紧张。若杂兵波次在 4 秒内清场，将有大量单位全程放不出战技。**列为首要验证项**，见[附录 B-7](#b-7-技能系统可能形同虚设)。
...
| 形态 | 条件 | 效果 |
|---|---|---|
| **大招** ⬜ | `Rage >= 100` 且有 `UltimateSkill` 且未沉默 | 按配置结算；`Rage → 0` |
...
v1.0 中硬编码在内核的六条天赋，现全部由羁绊档位经 `Modifier` 提供 ⬜ [D-08a](DESIGN_DECISIONS.md)：
...
| **中毒** `Poison` | 20s | 2s | 2% MaxHp | 20% |
| **流血** `Bleed` | 6s | 2s | 5% MaxHp | 15% |

> ⚠ 中毒持续 20 秒，而 ⬜ [D-04c](DESIGN_DECISIONS.md) 的目标战斗时长是 20~40 秒。**中毒很可能跑不完一个完整周期**，需重新评估其持续时间（建议缩短至 10s / 每跳 4%）。
...
⬜ [D-04d](DESIGN_DECISIONS.md)。**全自动下 AI 质量直接等于游戏质量**——玩家无法纠正 AI 的任何决策。
...
### 15.3 推荐强化方向 ⬜ [D-04d](DESIGN_DECISIONS.md)

| 层级 | 内容 |
|---|---|
| **A. 启发式强化** | AOE 技能优先选命中数最多的目标簇；治疗低于阈值才释放；单体技能优先选择"可击杀"目标 |
| **B. 战术指令配置**（推荐） | 战前可为每个素体配置 2 条战术指令（类似 FF12 Gambit），如「优先攻击血量最低的敌人」「自身血量 <30% 时优先自保」 |

**B 的设计意义**：全自动砍掉了实时操作，玩家需要一个**新的表达意图的出口**，否则会觉得"这游戏跟我没关系"。战术指令把操作从实时移到战前，与全自动定位完全兼容。

**范围控制**：首发每素体 2 个指令槽、指令库 12~16 条，避免变成编程作业。
...
| 词条 | ⬜ [D-09a](DESIGN_DECISIONS.md) 推荐**不锁定**，可拔下转给替补 |
| 局内失败 | 可上阵素体不足且无力救治 |
...
|---|
| 我方全灭 |
| 战斗计时器超过 `BattleTimeLimit`（⬜ 推荐 90s） |
...
| 27 | AI 是否留资源等时机 | 否，满即放（⬜ D-04d 建议改进） | 🟡 |
...
三人排合法列组合唯一（`{0,2,4}`）。成因是渲染遮挡。⬜ [D-07](DESIGN_DECISIONS.md)。
...
### B-14 中毒持续时间超出战斗时长 ⚠ 新增

中毒持续 20 秒，而 ⬜ [D-04c](DESIGN_DECISIONS.md) 目标战斗时长 20~40 秒。中毒很可能在杂兵战中**跑不完一个完整周期**，实际收益远低于账面。建议缩短至 10s / 每跳 4%。

### B-15 AI 质量成为体验瓶颈 ⚠ 新增

D-04 全自动后玩家无法纠正 AI 的任何决策，**AI 质量直接等于游戏质量**。15.2 列出的四项缺陷在手操版本中可被玩家绕过，现在全部暴露。⬜ [D-04d](DESIGN_DECISIONS.md)。
