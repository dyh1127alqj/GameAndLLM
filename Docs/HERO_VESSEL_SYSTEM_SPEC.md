...
|:---:|:---:|---|---|---|:---:|
| **Level 1** | **0 消耗**<br>(初始白送/流动) | **3 槽**：核心×2 + 作战×1 | 无转译能力<br>($f(x) = x$) | 无拦截权，纯响应回调 | 前期过渡垫脚石 |
| **Level 2** | **2 ~ 3 消耗**<br>(适度沉没) | **4 槽**：核心×2 + 作战×1 + 通用×1 | 单标签条件触发加成 | 单事件条件监听分支 | 流派基石与枢纽 |
| **Level 3** | **5 ~ 6 消耗**<br>(重大投资) | **5 槽**：核心×2 + 作战×2 + 通用×1 | 触发时机/结算域转译 | 独立计算乘区注入 | 战术核心建队支点 |
| **Level 4** | **8+ 或 特殊奇遇**<br>(极端昂贵) | **6 槽**：核心×2 + 作战×2 + 通用×1 + 奇点×1 | 跨世界形态强制同化 | 时间/空间/生死法则拦截 | 终局决胜引力源 |

> **插槽经济提示** ✅ [D-25](DESIGN_DECISIONS.md)：**核心槽恒为 2 个，不随权能等级增长**——职业与血脉共用这 2 个"身份槽"。因此即便是 Level 1 垫脚石也能完整参与羁绊凑件（这是刻意设计，避免低星素体被排除在构筑之外）。
>
> 权能等级买的是**广度与深度**，不是参与权：Level 1 只有 `2 身份 + 1 战技`，Level 4 有 `2 身份 + 2 战技 + 1 通用 + 1 奇点`，外加 Level 3~4 独享的词条转译与管线拦截权能。
>
