...
> ✅ **第二节三条世界法则的机制依赖已全部闭环**（D-10 / D-11 / D-12），见 [2.3](#23--世界法则的机制依赖检查)。
...
|:---:|---|---|:---:|---|:---:|
| **W-01** | **末日生化都市** | 废土、丧尸狂潮、辐射废墟、变异体 | `[生化变异]`<br>`[辐射废土]` | **环境辐射**：所有单位每 10 秒流失 2% 生命，`HealingReceived × 0.8` | ✗ |
| **W-02** | **赛博霓虹蜂巢** | 机械义体、数据流、巨企压迫、AI 神明 | `[赛博义体]`<br>`[超频协议]` | **电磁过载**：**技能伤害 +15%**，但释放技能后自身强制眩晕 0.5 秒（受 DR 约束） | **✓** |
...
> **首发范围** ✅ [D-03](DESIGN_DECISIONS.md) 已定案：首发 **W-02 赛博 + W-03 蜀山**——它们是四者中风格反差最强的一对，而跨界超武融合正是靠反差产生爽点（"赛博飞剑"成立，"末日 × 深渊"因基调相近而平淡）。
...
| W-03 | 法力自然回复 | ✅ 内核引入 `ManaRegenPerSec`（基础 0，由法则/词条激活） | [D-11](DESIGN_DECISIONS.md) |

> **W-04 的额外问题**：攻击力每秒 +1% 在 ✅ [D-04c](DESIGN_DECISIONS.md) 的 90 秒上限下最高 +90% 攻击 / +135% 受伤，处于 [D-13](DESIGN_DECISIONS.md) 软钳制区间内。**[补全]** 该计时器**不跨波次重置**（整场连续累加），使长线消耗战真正危险。
...
| 槽位类型 | 可装载 | 对应被动类别 |
|---|---|---|
| **核心槽（Core Socket）** | 【职业】或【血脉】词条 | 职业 / 血脉 | **每素体恒为 2 个** |
| **作战槽（Combat Socket）** | 武器、伤害机制、触发机制词条 | 技能 | 1 ~ 2 个 |
| **通用槽（Utility Socket）** | 属性增幅 / 机制改造（**不提供羁绊标签**） | — | 0 ~ 1 个 |
| **奇点槽（Singularity）** | 超武 / 概念级特性 | — | 0 ~ 1 个（Level 4 进阶阶） |

> **插槽经济** ✅ [D-25](DESIGN_DECISIONS.md)：核心槽**恒为 2 个**，职业与血脉共用。全队 `5 × 2 = 10 个身份槽`，使羁绊 (6) 档可达。完整数量矩阵见 [AFFIX_AND_SYNERGY 1.2](AFFIX_AND_SYNERGY.md)。
>
> **核心取舍**：`职业(6) + 血脉(4)` 恰好用满 10 个身份槽，是零冗余的理论最优构筑。
>
> ✅ [D-08b](DESIGN_DECISIONS.md) 已定案：白板素体**允许出战**；开局每名初始素体在核心槽**预装 1 枚 Lv1 职业词条**作为保底。
...
* **精炼（Refine）**：`2 枚同名同阶词条` $\to$ `1 枚高一阶词条`（数值提升 60%，并产生额外的**次级效果**）；

> ⚠ **精炼 ≠ 分解 ≠ 融合**，三者定义见 [AFFIX_AND_SYNERGY 5.0](AFFIX_AND_SYNERGY.md)。精炼是同名升阶，分解（Salvage）是换信用点，融合（Fuse）是跨界超武合成。
...
* **标准阶梯**：`(2)` 件激活微共鸣，`(4)` 件激活大共鸣。**血脉套装到 (4) 为止，没有 (6) 档。**

### 5.1.1 ✅ 为什么血脉只到 (4)，而职业羁绊有 (6)

这是**刻意的不对称**——两类词条共用同一批 10 个核心槽（[D-25](DESIGN_DECISIONS.md)），但奖励两种相反的投入形状：

| | 职业羁绊 | 血脉套装 |
|---|---|---|
| 阶梯 | `(2) / (4) / (6)` | `(2) / (4)` |
| 奖励的投入形状 | **纵深** —— 押 60% 身份槽换全队级质变 | **广度** —— 每套堆到 4 件即封顶，多余的投进下一套 |
| 典型构筑 | 职业 `6+4` | 血脉 `4+4` 或 `4+2` |

**血脉不需要 (6) 档，因为第 5 件同标签血脉的正确去处是另一套血脉**：
...
玩家永远不会把第 5 件投进同一套（收益为零），而是自然转向第二套——**取舍由阶梯结构自动引导，不需要额外规则**。

> **[补全]** 首发 2 个世界（✅ D-03）时，血脉的理论上限是 `4 + 4 = 8 件`；若 10 个核心槽全给血脉，最后 2 件会浪费。但那意味着完全放弃职业羁绊，不是有效构筑，无需为此设计兜底。扩展到 4 个世界后可分散至 4 套，上限自然提高。
...
| **4. 背包容量** | [D-16](DESIGN_DECISIONS.md) / [D-25](DESIGN_DECISIONS.md) | **15 格**（原 10 格，随核心槽扩容同比例上调）。需容纳至少 2 组待融合词条，但不能大到"什么都不用丢" |
