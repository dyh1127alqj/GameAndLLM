...
| **跨界裂隙** | **产出非本世界的异界词条**（另一首发世界的血脉/技能类）；兼作**超武融合**场所 | 纯 `RogueInventory` 状态修改 |
...
### 1.5 跨界裂隙的必要性与配比 ✅ [D-22](DESIGN_DECISIONS.md)

**为什么必须有这个节点**：单局 = 1 个世界（D-22），但**超武融合的 12 个配方全部是"赛博 × 蜀山"的跨界组合**（见 [AFFIX_AND_SYNERGY 第四节](AFFIX_AND_SYNERGY.md)）。若局内只能拿到本世界词条，**整个超武系统在任何一局里都无法使用**——而它正是无限流题材相对单一题材的核心增量。

| 项 | 值 |
|---|---|
| 词条掉落中异界词条占比 | **25%**（✅ D-22） |
| 跨界裂隙节点在单层中的出现 | 每层至少 1 个，可与位面异象共用泳道位 |
| 裂隙产出 | 另一首发世界的血脉 / 技能类词条；**不产出本世界词条** |
| 裂隙附加职能 | 可执行超武**融合（Fuse）** |

> **[补全]** 首发仅 2 个世界（✅ D-03），因此"异界"的指向是唯一的：在蜀山局产出赛博词条，在赛博局产出蜀山词条。未来扩展到 4 个世界时，需定义异界来源的随机权重。
...
| 备用词条背包容量 | [D-16](DESIGN_DECISIONS.md) / [D-25](DESIGN_DECISIONS.md) | 固定 **15 格**（随插槽扩容上调） | ✅ |
...
| **虚空清算组** | 密契行商 | 初始授权点 +4、备用背包扩至 **20 格**（基线 15 + 5）；救治重伤素体费用固定减免 30% | 快速展开阵容、容错容灾、全系混搭 |
